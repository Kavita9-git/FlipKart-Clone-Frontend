...
const OrderItem: FC<{ item: any, showAddButton?: boolean }> = ({ item, showAddButton = true }) => {
  return (
    <View style={styles.flexRow}>
      <View style={styles.imageContainer}>
        <Image source={{ uri: item?.image_uri }} style={styles.img} />
        {showAddButton && <UniversalAdd item={item} />}
      </View>
            <View style={styles.itemContainer}>
                <Text style={styles.itemName}>{item.name}</Text>
<Text style={styles.itemMeta}>Color: {item.color} | Size: {item.size}</Text>
